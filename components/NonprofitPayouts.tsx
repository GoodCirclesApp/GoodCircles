...
import { CreditCard, CheckCircle2, Clock, ArrowRight, ExternalLink, ShieldCheck, ShieldAlert } from 'lucide-react';
...
export const NonprofitPayouts: React.FC = () => {
  ...
  useEffect(() => {
    Promise.all([
      nonprofitService.getPayouts().catch(() => []),
      nonprofitService.getStripeStatus().catch(() => ({ isConnected: false })),
    ])
      .then(([p, s]: any) => {
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-12">
              ...
              {connected ? (
                ...
                <div className="px-6 py-3 bg-amber-50 text-amber-600 rounded-2xl border border-amber-100 flex items-center gap-3">
                  <ShieldAlert size={18} />
                  <span className="text-[10px] font-black uppercase tracking-widest">Connect your bank</span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                ...
                <p className="text-4xl font-black tracking-tighter text-[#7851A9]">{money(totalPending)}</p>
                <p className="text-[10px] font-medium text-slate-400 mt-4 italic">
                  {totalPending > 0 ? 'Scheduled for your next payout date' : 'No funds pending yet'}
                </p>
              </div>
              <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Total Paid Out</p>
                <p className="text-4xl font-black tracking-tighter">{money(totalPaid)}</p>
        ...
        <div className="bg-black text-white p-12 rounded-[4rem] shadow-2xl space-y-8">
          ...
          <p className="text-slate-400 text-xs font-medium leading-relaxed italic">
            {connected
              ...
              : 'Connect a bank account through Stripe to receive your donations automatically. Funds route to you as neighbors who chose your cause make purchases.'}
          </p>
          <div className="space-y-4 pt-4">
            <button className="w-full bg-white text-black py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#7851A9] hover:text-white transition-all flex items-center justify-center gap-3">
              {connected ? 'Go to Stripe Dashboard' : 'Connect Bank Account'}
      ...
      <div className="bg-white rounded-[3rem] border border-slate-100 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="py-16 px-8 text-center">
            <p className="text-[#7851A9] font-black uppercase tracking-widest">No payouts yet</p>
            <p className="text-slate-400 text-sm mt-2 max-w-md mx-auto">
              Your payout history will appear here once your community's contributions are disbursed to your connected
              bank account.
            </p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-50">
                {payouts.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-10 py-8 font-black text-lg">{money(Number(p.amount))}</td>
                    ...
                    <td className="px-10 py-8">
                      <div className="flex items-center gap-3">
                        {p.status === 'PAID' ? <CheckCircle2 className="text-emerald-500" size={16} /> : <Clock className="text-amber-500" size={16} />}
                        <span className={`text-[10px] font-black uppercase tracking-widest ${p.status === 'PAID' ? 'text-emerald-500' : 'text-amber-500'}`}>{p.status}</span>
                      </div>
                    </td>
                    <td className="px-10 py-8">
