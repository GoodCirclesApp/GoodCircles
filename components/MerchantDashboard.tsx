...
export const MerchantDashboard: React.FC<Props> = ({ onOpenAdvisor }) => {
  ...
  const isDemo = !metrics || (metrics.totalSales ?? 0) === 0; // "no sales yet" — show real zeros, never fabricated traction
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      {isDemo && (
        <div className="flex items-center gap-3 px-5 py-3 bg-amber-50 border border-amber-200 rounded-2xl w-fit">
          ...
          <p className="text-[10px] font-black uppercase tracking-widest text-amber-700">No sales yet — your live metrics appear here after your first sale</p>
        </div>
      )}

      {/* AI advisor CTA — real, data-grounded insights live in Margin Copilot; this is an honest prompt, not fabricated analysis */}
      <div className="bg-gradient-to-br from-[#7851A9]/5 to-[#CA9CE1]/5 border border-[#CA9CE1]/30 rounded-[1.5rem] p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-[#7851A9]/10 text-[#7851A9] rounded-xl flex items-center justify-center shrink-0">
            <Zap size={17} />
          </div>
          <div>
            <p className="text-sm font-black text-black">AI Advisor</p>
            <p className="text-[11px] text-slate-500 font-medium">
              {isDemo
                ? 'Insights activate after your first sales. Meanwhile, ask anything about pricing, margins, or the model.'
                : 'Ask about your pricing, margins, or growth — grounded in your own numbers.'}
            </p>
          </div>
        </div>
        <button
          onClick={() => onOpenAdvisor?.('How can I price my products for a healthy margin while maximizing the nonprofit donation under the Good Circles 10/10/1 model?')}
          className="shrink-0 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest bg-[#7851A9] text-white hover:bg-black transition-all"
        >
          Ask the AI Advisor →
        </button>
      </div>
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
              <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${isDemo ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
                {isDemo ? 'Awaiting first sale' : 'Live Data'}
              </span>
            </div>
          </div>
