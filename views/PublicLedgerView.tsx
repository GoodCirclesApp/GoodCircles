...
export const PublicLedgerView: React.FC<Props> = ({ orders, batches, globalStats, onToast }) => {
  ...
  // This ledger bills itself as a "verifiable real-time audit of every dollar" — so it must
  // show ONLY real settled orders. No seeded sample rows (they would be unverifiable fiction).
              ...
              {filteredOrders.length === 0 ? (
                <tr><td colSpan={6} className="py-20 text-center text-slate-300 font-bold italic uppercase text-xs">
                  {searchTerm
                    ...
                    : 'No settlements recorded yet — this stream populates with the community’s first verified transactions.'}
                </td></tr>
              ) : (
                <>
