  ...
  const handleReferral = (bizName: string) => {
    if (currentUser && onUpdateUser) {
      onUpdateUser({
        ...
        impactPoints: (currentUser.impactPoints || 0) + 25 // Referral bonus points
      });
    }
  };
    ...
    <div className="space-y-24 animate-in fade-in duration-700">
      ...
      <section className="space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {orders.length === 0 ? (
            <div className="col-span-full py-24 text-center border-2 border-dashed border-slate-100 rounded-[4rem]">
              <p className="text-slate-400 font-bold italic">No settlements recorded. Start your journey in the marketplace.</p>
            </div>
          ) : (
            orders.map(order => (
              <div key={order.id} className="p-10 bg-white rounded-[3.5rem] border border-[#CA9CE1]/20 shadow-xl group hover:shadow-2xl transition-all relative overflow-hidden">
