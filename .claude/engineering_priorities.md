...
- ☑ Prisma **migration baseline** + switch to `prisma migrate deploy`; remove `db push --accept-data-loss`. _Done 2026-06-16 (P1-A, commit 87c14de): verified ZERO prod drift (migrate diff empty), generated `0_init` baseline (97 tables / 6 enums / 164 indexes, 0 DROP/TRUNCATE), marked applied in prod via `migrate resolve --applied` (bookkeeping only — waitlist preserved), switched Dockerfile + package.json to `migrate deploy` (fail-closed `&&`). Pre-flighted against prod: "No pending migrations to apply." **Unblocks P1-B/C/D.**_
- ◐ Money columns → `Decimal` w/ explicit precision/scale; remove `Float` money (catalog/analytics); real Prisma enums; hot-path indexes. _Indexes DONE 2026-06-16 (P1-B1, commit 3740bbd): added Transaction(createdAt), Transaction(merchantId,createdAt), Transaction(paymentMethod), MerchantObligation(isSettled). REMAINING (P1-B2, HIGH risk, do last): Float→Decimal on analytics/catalog money cols; String→enum (esp. reconcile PaymentMethod vocabulary first)._
...
- ☑ Webhook/ledger **idempotency** (Stripe event-id dedupe). _Done 2026-06-16 (P1-D, commit f0bbc64): new `ProcessedWebhookEvent` table keyed `@@unique([eventId, source])`; both Stripe webhooks (payment + catalog) check-at-start / mark-at-end → redelivered events skipped, failed events left unmarked so Stripe retries reprocess cleanly. Known limit: a hard process crash between mark-success and a second concurrent delivery (rare for Stripe's sequential retries)._
- ☑ Persist Stripe charge/payment-intent id on `Transaction` (enables refunds/reconciliation). _Done 2026-06-16 (P1-C, commit 3740bbd): nullable stripeCheckoutSessionId/stripePaymentIntentId/stripeChargeId; session id set at checkout, PI id set in checkout.session.completed webhook. Unblocks real Stripe refund execution (refundService STRIPE branch now has the id)._
