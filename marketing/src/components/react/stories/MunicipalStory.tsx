...
export default function MunicipalStory({ onConfirm, onBack }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      onConfirm({ position: res.position, inviteCode: res.inviteCode, overflow: res.overflow, alreadyRegistered: res.alreadyRegistered, role: 'MUNICIPAL', email });
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div {...fadeUp(0.05)} className="mb-14">
          ...
          <p className="text-lg text-slate-600 mb-10 leading-relaxed" style={{ fontFamily: "'Fira Sans', sans-serif" }}>
            Good Circles keeps dollars circulating locally, surfaces the small businesses that are
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              { icon: <Building2 size={20} />, title: 'No new infrastructure', body: 'Good Circles runs independently. Municipal partners connect their dashboard — nothing to build or maintain.' },
            ].map((card, i) => (
        ...
        <motion.div {...fadeUp(0.4)}>
          <div className="p-8 rounded-3xl" style={{ background: '#fff', boxShadow: `0 4px 40px ${COLOR}18` }}>
            ...
            <p className="text-slate-500 mb-8" style={{ fontFamily: "'Fira Sans', sans-serif" }}>
              ...
              Founding municipal partners shape how Good Circles serves their community.
            </p>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <input type="email" aria-label="Your government email" placeholder="Your government email" value={email} onChange={e => setEmail(e.target.value)} required
              ...
              <input type="text" aria-label="City or county name" placeholder="City or county name" value={jurisdiction} onChange={e => setJurisdiction(e.target.value)}
                className={`w-full px-8 py-5 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-4 ${RING} text-base font-medium placeholder-slate-400 transition-all`} />
              <input type="text" aria-label="Your role (e.g. Economic Development Director)" placeholder="Your role (e.g. Economic Development Director)" value={decisionMakerRole} onChange={e => setDecisionMakerRole(e.target.value)}
                className={`w-full px-8 py-5 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-4 ${RING} text-base font-medium placeholder-slate-400 transition-all`} />
