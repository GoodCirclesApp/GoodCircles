...
const COMPARISON = [
  ...
  { platform: 'Good Circles',  youKeep: '89%',   note: 'Of net profit + tax benefit', highlight: true },
];
...
export default function MerchantStory({ onConfirm, onBack }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      onConfirm({ position: res.position, inviteCode: res.inviteCode, overflow: res.overflow, alreadyRegistered: res.alreadyRegistered, role: 'MERCHANT', email });
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <motion.div {...fadeUp(0.05)} className="mb-14">
          ...
          <p className="text-lg text-slate-600 mb-10 leading-relaxed" style={{ fontFamily: "'Fira Sans', sans-serif" }}>
            ...
            charitable contribution on your behalf. 1¢ is Good Circles' platform fee.
        ...
        <motion.div {...fadeUp(0.3)} className="mb-14 grid sm:grid-cols-3 gap-4">
          {[
            { icon: <Users size={22} />,      title: 'Community-loyal buyers',    body: 'Neighbors save about 10% by shopping Good Circles — so they\'re actively looking for businesses like yours.' },
        ...
        <motion.div {...fadeUp(0.5)}>
          <div className="p-8 rounded-3xl" style={{ background: '#fff', boxShadow: `0 4px 40px ${COLOR}20` }}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <input type="email" aria-label="Your email address" placeholder="Your email address" value={email} onChange={e => setEmail(e.target.value)} required
              ...
              <input type="text" aria-label="Business name" placeholder="Business name" value={businessName} onChange={e => setBusinessName(e.target.value)} required
                className={`w-full px-8 py-5 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-4 ${RING} text-base font-medium placeholder-slate-400 transition-all`} />
              <div className="grid sm:grid-cols-2 gap-4">
                <input type="text" aria-label="City" placeholder="City" value={city} onChange={e => setCity(e.target.value)}
                  className={`w-full px-8 py-5 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-4 ${RING} text-base font-medium placeholder-slate-400 transition-all`} />
                <input type="text" aria-label="Business type (e.g. Food, Retail)" placeholder="Business type (e.g. Food, Retail)" value={category} onChange={e => setCategory(e.target.value)}
                  className={`w-full px-8 py-5 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-4 ${RING} text-base font-medium placeholder-slate-400 transition-all`} />
              </div>
